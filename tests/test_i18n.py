...
def test_translation_lookup_and_format():
    ...
    assert translator.t('selector.button.quit') == 'Quitter'
    title = translator.t('selector.title', version='1.2.3')
    assert '1.2.3' in title


def test_missing_key_returns_key():
    translator.set_language('fr')
    assert translator.t('non.existing.key') == 'non.existing.key'


def test_set_language_switch():
    assert translator.set_language('en') is True
    assert translator.t('selector.button.quit') == 'Quit'
...
def test_malformed_json_handled_gracefully():
    ...
    try:
        ...
        assert translator.t('selector.button.quit') == 'selector.button.quit'
    finally:
        # Nettoyage
        try:
            if bad_file.exists():
